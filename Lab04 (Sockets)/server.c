...
int main(int argc, char *argv[])
{
    ...
    /* Populate the sockaddr_un struct 
	 */
    saun.sun_family = AF_UNIX;
    strcpy(saun.sun_path, SOCKET_ADDRESS);
 
    /* Create the handshake socket 
	 */
    ...
    /* 
     * Unlink the address before binding to ensure the
     * address is free before attempting to bind.
     */
    unlink(SOCKET_ADDRESS);

    /* Bind the handshake socket to the sockaddr. */ 
    ...
    /* Make the handshake socket a listening socket, with a
    ...
    /* Accept a connection on the handshake socket,
    ...
    /* Read lines one at a time from the connected session
